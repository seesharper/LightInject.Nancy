...
private const string projectName = "LightInject.Nancy";

private const string portableClassLibraryProjectTypeGuid = "{786C830F-07A1-408B-BD7F-6EE04809D6DB}";
private const string csharpProjectTypeGuid = "{FAE04EC0-301F-11D3-BF4B-00C04F79EFBC}";

string pathToSourceFile = @"..\src\LightInject.Nancy\LightInject.Nancy.cs";
...
WriteLine("LightInject.Nancy version {0}" , version);
...
{	    
	string pathToMetadataFile = Path.Combine(pathToBuildDirectory, "NugetPackages/Source/package/LightInject.Nancy.Source.nuspec");	
...
{	    
	string pathToMetadataFile = Path.Combine(pathToBuildDirectory, "NugetPackages/Binary/package/LightInject.Nancy.nuspec");
	PatchNugetVersionInfo(pathToMetadataFile, version);
	NuGet.CreatePackage(pathToMetadataFile, pathToBuildDirectory);
}
...
{
	string pathToMetadata = "../src/LightInject.Nancy/NuGet";
	string pathToPackageDirectory = Path.Combine(pathToBuildDirectory, "NugetPackages/Source/package");	
	RoboCopy(pathToMetadata, pathToPackageDirectory, "LightInject.Nancy.Source.nuspec");	
	string pathToSourceFile = "tmp/" + frameworkMoniker + "/Source/LightInject.Nancy";
	string pathToDestination = Path.Combine(pathToPackageDirectory, "content/" + packageDirectoryName + "/LightInject.Nancy");
	RoboCopy(pathToSourceFile, pathToDestination, "LightInject.Nancy.cs");
	FileUtils.Rename(Path.Combine(pathToDestination, "LightInject.Nancy.cs"), "LightInject.Nancy.cs.pp");
	ReplaceInFile(@"namespace \S*", "namespace $rootnamespace$.LightInject.Nancy", Path.Combine(pathToDestination, "LightInject.Nancy.cs.pp"));
}
...
private void CopyBinaryFile(string frameworkMoniker, string packageDirectoryName)
{
	...
	RoboCopy(pathToMetadata, pathToPackageDirectory, "LightInject.Nancy.nuspec");
	...
	RoboCopy(pathToBinaryFile, pathToDestination, "LightInject.Nancy.*");
}

private string ResolvePathToBinaryFile(string frameworkMoniker)
{
	var pathToBinaryFile = Directory.GetFiles("tmp/" + frameworkMoniker + "/Binary/LightInject.Nancy/bin/Release","LightInject.Nancy.dll", SearchOption.AllDirectories).First();
...
{
	string pathToProjectFile = Path.Combine(pathToBuildDirectory, @"netstandard16/Binary/LightInject.Nancy/project.json");
	DotNet.Build(pathToProjectFile, "netstandard1.6");
}
...
{
	string pathToProjectDirectory = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Binary/LightInject.Nancy");
	NuGet.Restore(pathToProjectDirectory);
	pathToProjectDirectory = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Binary/LightInject.Nancy.Tests");
	NuGet.Restore(pathToProjectDirectory);
	pathToProjectDirectory = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Source/LightInject.Nancy");
	NuGet.Restore(pathToProjectDirectory);
	pathToProjectDirectory = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Source/LightInject.Nancy.Tests");
	NuGet.Restore(pathToProjectDirectory);
...
{	
	...
	Execute(() => RunUnitTests("Net452"), "Running unit tests for Net452");
...
{
	string pathToTestAssembly = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Binary/LightInject.Nancy.Tests/bin/Release/LightInject.Nancy.Tests.dll");
...
{	
	Execute(() => AnalyzeTestCoverage("NET452"), "Analyzing test coverage for NET452");
...
{	
    ...
    MsTest.RunWithCodeCoverage(pathToTestAssembly, pathToPackagesFolder, "LightInject.Nancy.dll");
}
...
private void InitializeNugetBuildDirectory(string frameworkMoniker)
{
	...
	{
		var pathToJsonTemplateFile = Path.Combine(pathToBinary, "LightInject.Nancy/project.json_");
		var pathToJsonFile = Path.Combine(pathToBinary, "LightInject.Nancy/project.json");
		File.Move(pathToJsonTemplateFile, pathToJsonFile);
		pathToJsonTemplateFile = Path.Combine(pathToSource, "LightInject.Nancy/project.json_");
		pathToJsonFile = Path.Combine(pathToSource, "LightInject.Nancy/project.json");
		File.Move(pathToJsonTemplateFile, pathToJsonFile);
	}				  
}
...
private void Internalize(string frameworkMoniker)
{
	...
	string pathToSourceFile = Path.Combine(pathToBuildDirectory, frameworkMoniker + "/Source/LightInject.Nancy/LightInject.Nancy.cs");
	Internalizer.Internalize(pathToSourceFile, frameworkMoniker, exceptTheseTypes);
}

private void InternalizeSourceVersions()
{
	Execute (()=> Internalize("NET452"), "Internalizing NET452");
...
{
	PatchPackagesConfig("net452");
	PatchPackagesConfig("net46");	
}

private void PatchPackagesConfig(string frameworkMoniker)
{
	string pathToPackagesConfig = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Binary/LightInject.Nancy/packages.config");
	ReplaceInFile(@"(targetFramework=\"").*(\"".*)", "$1"+ frameworkMoniker + "$2", pathToPackagesConfig);
	
	pathToPackagesConfig = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Source/LightInject.Nancy/packages.config");
	ReplaceInFile(@"(targetFramework=\"").*(\"".*)", "$1"+ frameworkMoniker + "$2", pathToPackagesConfig);
}

private void PatchAssemblyInfo()
{
	Execute(() => PatchAssemblyInfo("Net452"), "Patching AssemblyInfo (Net452)");
...
{	
	var pathToAssemblyInfo = Path.Combine(pathToBuildDirectory, framework + @"\Binary\LightInject.Nancy\Properties\AssemblyInfo.cs");	
	PatchAssemblyVersionInfo(version, fileVersion, framework, pathToAssemblyInfo);
	pathToAssemblyInfo = Path.Combine(pathToBuildDirectory, framework + @"\Source\LightInject.Nancy\Properties\AssemblyInfo.cs");
	PatchAssemblyVersionInfo(version, fileVersion, framework, pathToAssemblyInfo);	
...
private void PatchInternalsVisibleToAttribute(string pathToAssemblyInfo)
{
	...
	sb.AppendLine(@"[assembly: InternalsVisibleTo(""LightInject.Nancy.Tests"")]");
	WriteFile(pathToAssemblyInfo, sb.ToString());
}

private void PatchProjectFiles()
{
	Execute(() => PatchProjectFile("NET452", "4.5.2"), "Patching project file (NET452)");
...
{
	var pathToProjectFile = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Binary/LightInject.Nancy/LightInject.Nancy.csproj");
	PatchProjectFile(frameworkMoniker, frameworkVersion, pathToProjectFile);
	pathToProjectFile = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Source/LightInject.Nancy/LightInject.Nancy.csproj");
	PatchProjectFile(frameworkMoniker, frameworkVersion, pathToProjectFile);
...
private void SetHintPath(string frameworkMoniker, string pathToProjectFile)
{
	...
	if (frameworkMoniker == "NET452") //LightInject is built against 4.5
	{
...
private void SetTargetFrameworkProfile()
{
	...
	var pathToProjectFile = Path.Combine(pathToBuildDirectory, @"PCL_111/Binary/LightInject.Nancy/LightInject.Nancy.csproj");
	XDocument xmlDocument = XDocument.Load(pathToProjectFile);	
...
{
	var pathToProjectFile = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"\Binary\LightInject.Nancy.Tests\LightInject.Nancy.Tests.csproj");
	WriteLine("Patching {0} ", pathToProjectFile);	
	SetProjectFrameworkMoniker(frameworkMoniker, pathToProjectFile);
	pathToProjectFile = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"\Source\LightInject.Nancy.Tests\LightInject.Nancy.Tests.csproj");
	WriteLine("Patching {0} ", pathToProjectFile);
	SetProjectFrameworkMoniker(frameworkMoniker, pathToProjectFile);
}